                ...
                {variantsList.map((v) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2.5 text-xs uppercase tracking-[0.18em] border flex items-center gap-2 transition-all duration-200 ${
                        isSelected
            ...
            {[
              { t: "Specifications", c: (
                <ul className="text-sm text-muted-foreground space-y-1">
                  {Object.entries(product.specs ?? {}).filter(([k, v]) => k !== "colors" && !Array.isArray(v) && typeof v !== "object").map(([k, v]) => <li key={k}><span className="text-foreground">{k}:</span> {v as string}</li>)}
                </ul>
              )},
